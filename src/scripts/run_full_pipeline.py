...
def _build_overview_pdf(df) -> bytes:
    ...
    # ── Cover ─────────────────────────────────────────────────────────────────
    story.append(Paragraph("Feedback Intelligence Report", title_s))
    ...
    if "date" in df.columns:
        ...
        story.append(Paragraph("Sentiment Over Time (Monthly)", h2_s))
        try:
            d = df.copy()
    ...
    story.append(Paragraph(
        ...
        "Sentiment predictions use Logistic Regression trained on labelled review data. "
        "Themes assigned by a locally-hosted LLM (Ollama / Gemma). "
        "Review results alongside raw data before making business decisions.", body_s))
